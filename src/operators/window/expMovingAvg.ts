 ...
 */
export let $expMovingAvg = (
  _: AnyObject,
  collection: AnyObject[],
  expr: WindowOperatorInput,
  options: Options
): Any => {
  let { input, N, alpha } = expr.inputExpr as {
    input: Any;
    N: number;
    alpha: number;
  };
  ...
  return withMemo(
    ...
    expr,
    () => {
      let series = $push(collection, input, options).filter(isNumber);
    ...
    (series: number[]) => {
      ...
      let weight = N != undefined ? 2 / (N + 1) : alpha;
      let i = expr.documentNumber - 1;
